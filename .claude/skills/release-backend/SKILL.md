...
description: 把 Pika 后端代码发布(部署)到绿联 NAS 的 Docker 项目目录。当用户说"release后端""部署后端""更新NAS上的服务""发版"时使用。会把最新的 Dockerfile + backend/ + docker-compose.yml 同步到 NAS 共享目录，保护 NAS 上已存在的 .env，并指引在绿联界面重新部署、验证 /health。
---
...
3. **同步代码到 NAS**（覆盖式复制，但排除数据/虚拟环境/git）：
   ...
   - compose 文件处理：
     - 若 NAS 已有 `docker-compose.yaml` 且结构正确（`${...}` 占位 + 资源限制），优先保留它，仅在仓库 compose 有实质结构变化时才更新；
     - 更新时保持 `environment` 全部用 `${...}`，不写明文密钥。
4. **校验 NAS 目录结构**：
   - `find //DH4300PLUS-D2E0/docker/pika -type f` 确认 `Dockerfile`、`backend/app/main.py`、`docker-compose.yaml`、`.env` 都在。
   - `grep -nE 'PUT_YOUR|<真实key特征>' docker-compose.yaml` 确认 compose 无明文密钥。
5. **DB 迁移提醒**：
