...
- item_name 必须优先返回报告单里出现的中文指标名；只有当该条指标在报告单中确实没有中文名称时，才允许返回英文名称或缩写。
- 若同一行同时有中英名称（如“钾 K”“谷丙转氨酶 ALT”），item_name 必须写中文，英文放 item_code。
- 若同一行是“中文 + 化学符号/缩写”（如 钾/K、钠/Na、氯/Cl、二氧化碳/CO2、谷丙转氨酶/ALT、谷草转氨酶/AST、总胆红素/TBIL、直接胆红素/DBIL、γ-谷氨酰转肽酶/GGT），item_name 一律输出中文名，不要输出 K/Na/Cl/CO2/ALT/AST/TBIL/DBIL/GGT 作为 item_name。
- 如果该行只有英文或缩写但同一报告其他位置给出对应中文（表头、注释、对照列），也要回填中文到 item_name，缩写写入 item_code。
