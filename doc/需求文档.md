...
1.3 功能需求
核心工作流（健康数据场景）
用户iPhone快捷指令上传好轻APP截图 → 调用Pika API → MasterAgent意图识别 → 
调用HealthMetricsAgent → 从图片直接读取斤数 → 更新Notion数据库
...
json{
  ...
  "parameters": {
    "storage_key": "{加密的存储密钥}",
    "blob_path": "health/2026/01/20260117_0830.png",
    "date": "2026-01-17"
  }
}
...
重要变更：好轻APP显示的体重单位本身就**是斤**，因此不再需要从公斤转换为斤的步骤
处理流程：
1. 从截图提取体重（斤）、体脂率(%)、肌肉率(%)、BMI
2. 模型直接从图片读取斤数（无需单位转换）
3. 更新到特定结构的Notion数据库